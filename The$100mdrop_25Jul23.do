clear all
import excel "/Users/nattavudhpowdthavee/Library/CloudStorage/Dropbox/The 100k drop/Data_1milliondollar.xlsx", sheet("Sheet1") firstrow

global out_dir "/Users/nattavudhpowdthavee/Dropbox/The 100k drop/Results"

**The $100m drop project
...
save "/Users/nattavudhpowdthavee/Dropbox/The 100k drop/The$100mdrop.dta", replace
...
use "/Users/nattavudhpowdthavee/Dropbox/The 100k drop/The$100mdrop.dta", clear


**Regression
*Table 1: Absolute loss 
xi:xtreg d_index loss remaining_t_1  $control1,   fe i(id) cluster(id)
outreg2 using "${out_dir}/table1_$100m_July23.xls", replace stat(coef se) 2aster  label dec(3)
xi:xtreg d_index loss loss_sq remaining_t_1  $control1,   fe i(id) cluster(id)
outreg2 using "${out_dir}/table1_$100m_July23.xls", append stat(coef se) 2aster  label dec(3)
...
outreg2 using "${out_dir}/table2_$100m_July23.xls", replace stat(coef se) 2aster  label dec(3)
xi:xtreg d_index loss_prop loss_prop_sq remaining_t_1  $control1,   fe i(id) cluster(id)
outreg2 using "${out_dir}/table2_$100m_July23.xls", append stat(coef se) 2aster  label dec(3)
...
xi:xtreg d_index loss l.loss  remaining_t_1  $control1,   fe i(id) cluster(id)
outreg2 using "${out_dir}/table3_$100m_July23.xls", replace stat(coef se) 2aster  label dec(3)
xi:xtreg d_index loss_prop l.loss_prop remaining_t_1  $control1,   fe i(id) cluster(id)
outreg2 using "${out_dir}/table3_$100m_July23.xls", append stat(coef se) 2aster  label dec(3)
...
outreg2 using "${out_dir}/table4_$100m_July23.xls", replace stat(coef se) 2aster  label dec(3)
xi:xtreg d_index loss remaining_t_1 d_index_t_1 $control0 if mgender==0.5,   fe i(id) cluster(id)
outreg2 using "${out_dir}/table4_$100m_July23.xls", append stat(coef se) 2aster  label dec(3)
...
outreg2 using "${out_dir}/table5_$100m_July23.xls", replace stat(coef se) 2aster  label dec(3)
xi:xtreg d_index loss remaining_t_1 d_index_t_1 $control0 if question>3 & question<=6,   fe i(id) cluster(id)
outreg2 using "${out_dir}/table5_$100m_July23.xls", append stat(coef se) 2aster  label dec(3)
 
 
*Table 6: All out 
xi:xtlogit all_out loss remaining_t_1 d_index_t_1 $control0,   fe i(id)  
outreg2 using "${out_dir}/table6_$100m_July23.xls", replace stat(coef se) 2aster  label dec(3)
xi:xtreg max_bet  loss remaining_t_1 d_index_t_1 $control0,   fe i(id)  
outreg2 using "${out_dir}/table6_$100m_July23.xls", append stat(coef se) 2aster  label dec(3)
