...
***Revision request
**Estimate without couple fixed effects
*Table 1: Absolute loss 
xi:xtreg d_index loss remaining_t_1  $control0,  fe i(id) cluster(id)
outreg2 using "${out_dir}/table3_revised.xls", replace stat(coef se) 2aster  label dec(3)
xi:xtreg d_index loss loss_sq remaining_t_1  $control0,   fe i(id) cluster(id)
outreg2 using "${out_dir}/table3_revised.xls", append stat(coef se) 2aster  label dec(3)
xi:xtreg d_index i.loss_cat2 remaining_t_1  $control0,  fe i(id) cluster(id)
outreg2 using "${out_dir}/table3_revised.xls", append stat(coef se) 2aster  label dec(3)
xi:xtreg d_index lg_loss remaining_t_1  $control0,   fe i(id) cluster(id)
outreg2 using "${out_dir}/table3_revised.xls", append stat(coef se) 2aster  label dec(3)
   
*Table 2: Loss proportion 
xi:xtreg d_index loss_prop remaining_t_1  $control0,  fe i(id) cluster(id)
outreg2 using "${out_dir}/table4_revised.xls", replace stat(coef se) 2aster  label dec(3)
xi:xtreg d_index loss_prop loss_prop_sq remaining_t_1  $control0,   fe i(id) cluster(id)
outreg2 using "${out_dir}/table4_revised.xls", append stat(coef se) 2aster  label dec(3)
xi:xtreg d_index i.loss_cat3 remaining_t_1  $control0,    fe i(id) cluster(id)
outreg2 using "${out_dir}/table4_revised.xls", append stat(coef se) 2aster  label dec(3)
xi:xtreg d_index lg_loss_prop remaining_t_1  $control0,   fe i(id) cluster(id)
outreg2 using "${out_dir}/table4_revised.xls", append stat(coef se) 2aster  label dec(3)

 
**Allowing for non-linearity in the remaining_t_1 
xi:xtreg d_index loss remaining_t_1*  $control1,  fe i(id) cluster(id)
outreg2 using "${out_dir}/table_remaining_nonl.xls", replace stat(coef se) 2aster  label dec(3)
xi:xtreg d_index loss loss_sq remaining_t_1*  $control1,   fe i(id) cluster(id)
outreg2 using "${out_dir}/table_remaining_nonl.xls", append stat(coef se) 2aster  label dec(3)
xi:xtreg d_index i.loss_cat2 remaining_t_1*  $control1,  fe i(id) cluster(id)
outreg2 using "${out_dir}/table_remaining_nonl.xls", append stat(coef se) 2aster  label dec(3)
xi:xtreg d_index lg_loss remaining_t_1*  $control1,   fe i(id) cluster(id)
outreg2 using "${out_dir}/table_remaining_nonl.xls", append stat(coef se) 2aster  label dec(3)
